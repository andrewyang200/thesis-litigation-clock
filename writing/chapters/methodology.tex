...
Fine-Gray model asks how covariates shift the cumulative
...
on the pre-PSLRA sample only (1990--1995). A significant placebo effect would indicate either that the model detects pre-existing trends or that a genuine structural break preceded the PSLRA, both of which would cast doubt on a clean PSLRA identification; an insignificant placebo effect would weaken
...
Because the PSLRA represents a sharp legislative cutoff rather than a
case-specific treatment, the propensity score here captures temporal shifts
in case composition across eras, not case-level selection into the
post-PSLRA regime. To adjust for these compositional differences, we
reweight the pre-PSLRA sample so that its observable characteristics
(filing circuit, case origin, and statutory basis) more closely resemble
those of the post-PSLRA era, separating the era difference from concurrent
shifts in the underlying case mix.

Formally, let $Z_i \in \{0,1\}$ denote the post-PSLRA indicator. The
propensity score is defined as
\begin{equation}
e(\mathbf{X}_i) = P(Z_i = 1 \mid \mathbf{X}_i),
\end{equation}
and is estimated via logistic regression of $Z_i$ on circuit, origin,
jurisdictional basis, and statutory basis.
...
with weight $w_i$; robust (sandwich) standard errors account
...
We interpret IPTW-weighted hazard ratios strictly as composition-adjusted estimates. They compare pre- and post-PSLRA cases under a common observable case mix. We do not interpret them as causal effects of the PSLRA.

Valid causal inference using IPTW requires three assumptions, each of which is difficult to satisfy here:
\begin{enumerate}
...
All models condition on the observed administrative covariates available in the IDB. However, unobserved factors may still affect both the likelihood and timing of outcomes, and more granular clustering at the judge level is not feasible because the database lacks consistent judge identifiers. The substantive implications are discussed in Chapter~\ref{ch:discussion}.
