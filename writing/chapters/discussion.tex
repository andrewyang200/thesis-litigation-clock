...
first year, post-PSLRA cases face an 86\% higher
dismissal hazard than pre-PSLRA cases (HR $= 1.859$). This is
...
The null effect in years one to two (HR $= 1.044$, $p >
0.05$) is equally important. Cases that survive the
...
The re-emergence of a modest PSLRA effect beyond two years
(HR $= 1.293$) may reflect differences in summary judgment
...
The robustness analysis provides important bounds on the
settlement effect, but also reveals a significant caveat.
Under the most liberal coding scheme (Scheme~B), the PSLRA
settlement HR rises from 0.378 to 0.696, still representing
a 30\% reduction in settlement hazard. The true effect lies
within the interval $[0.378, 0.696]$ depending on the
fraction of Code~12 cases representing private settlements.
However, when flexible time-trend controls (a natural spline
with $df{=}3$) absorb secular changes in judicial behavior,
the settlement HR attenuates further to 0.748 and loses
statistical significance ($p = 0.076$). This suggests that
part of the raw settlement suppression attributed to the
PSLRA in the baseline specification reflects concurrent
secular trends in litigation patterns rather than the reform
alone. The settlement finding is therefore robust to
disposition coding choices but sensitive to time-trend
specification, and should be interpreted with appropriate
caution.
...
(interaction HR $= 0.376$) and the Ninth Circuit's
attenuation (HR $= 0.703$) point to meaningful variation
...
cases show a modestly higher cumulative settlement probability
(SHR $= 1.126$, 95\% CI $[0.795, 1.595]$, $p = 0.503$). Because bilateral suppression of both
hazards keeps large numbers of MDL cases in the risk set
throughout the study period, cumulative settlement
probability accumulates to a similar---or slightly higher---level as
non-MDL cases, though this difference is not statistically significant. This finding would be completely invisible
...
\citep{wang2022}. While we implement circuit-level shared frailty models
to account for clustering at the circuit level, the IDB lacks consistent
judge identifiers or attorney data, so finer-grained clustering at the
judge or counsel level remains unmodeled. Consequently, unobserved
judicial or counsel quality may confound some of our observed circuit
or origin effects. Finally, our reliance on the 
